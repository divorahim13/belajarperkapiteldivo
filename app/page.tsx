...
function Flashcards({ direction }: { direction: FlashcardDirection }) {
  ...
  const liveDragTransform = dragOffset
    ? `translate3d(${dragOffset}px, ${Math.abs(dragOffset) * -0.03}px, 42px) rotateZ(${dragOffset * 0.055}deg) rotateY(${dragOffset * -0.09}deg) scale(${1 - Math.min(Math.abs(dragOffset) / 2400, 0.045)})${flipped ? " rotateY(180deg)" : ""}`
    : undefined;
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    const easedDistance = Math.sign(distance) * Math.min(Math.abs(distance), 190);
